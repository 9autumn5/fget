...
class Model(object):
    ...
    def context(self):
        ...
        return tf.concat(1, [left_weighted / left_all, right_weighted / right_all])  # 200+200=400
...
class SA(Model):
    ...
    def predict(self):
        x = tf.concat(1, [self.entity(), self.context()])  # ? , 700
    ...
    def fdict(self, w2v, now, size, interval, _entity, _context, _label, _fbid, _embedding, _test):
        ...
        fd = {}
        new_size = int(size / interval)  # 1000   batch

        ent = np.zeros([new_size, self.word_size])  # 1000 300
        lab = np.zeros([new_size, self.type_size])  # 1000  74
        for i in range(new_size):
            vec = np.zeros([self.word_size])
            l = len(_entity[now + i * interval])  # 2  'Bosnia' 'war'
            for j in range(l):
        ...
        for j in range(self.window):
            left_con = np.zeros([new_size, self.word_size])  # 1000  300
            right_con = np.zeros([new_size, self.word_size])  # 1000  300
            for i in range(new_size):
                left_con[i, :] = util.dic(w2v, _context[now + i * interval][2 * j])
                right_con[i, :] = util.dic(w2v, _context[now + i * interval][2 * j + 1])
            fd[self.left_in[j]] = left_con  # 循环内的j
...
class KA(Model):
    ...
    def predict(self):
        ...
        train1 = tf.train.AdamOptimizer(0.005).minimize(self.loss1)

        self.loss2 = tf.reduce_sum(tf.pow(self.query - self.query_, 2))
        train2 = tf.train.AdamOptimizer(0.005).minimize(self.loss2)
...
class KA_D(Model):
    ...
    def __init__(self, name):
        ...
        self.disamb = util.build_disamb("data/disamb_file")
